...
CMLTSampler::CMLTSampler(const CMLTConfiguration &config) : Sampler(Properties()) {
	m_random = new Random();
...
CMLTSampler::CMLTSampler(CMLTSampler *sampler) : Sampler(Properties()),
...
CMLTSampler::CMLTSampler(Stream *stream, InstanceManager *manager)
...
void CMLTSampler::serialize(Stream *stream, InstanceManager *manager) const {
...
CMLTSampler::~CMLTSampler() { }

void CMLTSampler::accept() {
	if (m_largeStep)
...
void CMLTSampler::reset() {
	m_time = m_sampleIndex = m_largeStepTime = 0;
	m_u.clear();
}

void CMLTSampler::reject() {
...
Float CMLTSampler::primarySample(size_t i) {
	while (i >= m_u.size())
...
ref<Sampler> CMLTSampler::clone() {
	ref<CMLTSampler> sampler = new CMLTSampler(this);
...
Float CMLTSampler::next1D() {
	return primarySample(m_sampleIndex++);
}

Point2 CMLTSampler::next2D() {
...
std::string CMLTSampler::toString() const {
	std::ostringstream oss;
	oss << "CMLTSampler[" << endl
		<< "  sampleCount = " << m_sampleCount << endl
		<< "]";
	return oss.str();
}

MTS_IMPLEMENT_CLASS_S(CMLTSampler, false, Sampler)
MTS_NAMESPACE_END
