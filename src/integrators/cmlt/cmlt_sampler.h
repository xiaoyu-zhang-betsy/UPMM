...
#if !defined(__CMLT_SAMPLER_H)
#define __CMLT_SAMPLER_H
 ...
 */
class CMLTSampler : public Sampler {
public:
	// Construct a new MLT sampler
	CMLTSampler(const CMLTConfiguration &conf);

	/**
	 * \brief Construct a new sampler, which operates on the
	 * same random number generator as \a sampler.
	 */
	CMLTSampler(CMLTSampler *sampler);

	/// Unserialize from a binary data stream
	CMLTSampler(Stream *stream, InstanceManager *manager);
