...
#if !defined(__GUPM_PROC_H)
#define __GUPM_PROC_H
...
class GuidedUPMProcess : public ParallelProcess {
public:
	GuidedUPMProcess(const RenderJob *parent, RenderQueue *queue, const GuidedUPMConfiguration &config);
...
protected:
	/// Virtual destructor
	virtual ~GuidedUPMProcess() { }
private:
	ref<const RenderJob> m_job;
	RenderQueue *m_queue;
	const GuidedUPMConfiguration &m_config;
