 ...
 */
class GuidedBDPTProcess : public BlockedRenderProcess {
public:
	GuidedBDPTProcess(const RenderJob *parent, RenderQueue *queue,
		const GuidedBDPTConfiguration &config);

	inline const GuidedBDPTWorkResult *getResult() const { return m_result.get(); }
...
protected:
	/// Virtual destructor
	virtual ~GuidedBDPTProcess() { }
private:
	ref<GuidedBDPTWorkResult> m_result;
