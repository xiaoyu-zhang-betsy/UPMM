...
* }
*/
class GuidedUPMIntegrator : public Integrator {
public:
	GuidedUPMIntegrator(const Properties &props) : Integrator(props) {
	...
	GuidedUPMIntegrator(Stream *stream, InstanceManager *manager)
	 : Integrator(stream, manager) {
		m_config = GuidedUPMConfiguration(stream);
		configure();
	}

	virtual ~GuidedUPMIntegrator() { }
...
MTS_EXPORT_PLUGIN(GuidedUPMIntegrator, "Guided unbiased photon mapping");
MTS_NAMESPACE_END
