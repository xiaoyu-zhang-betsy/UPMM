...
/*!\plugin{epssmlt}{Extendted Primary Sample Space Metropolis Light Transport}
 * \order{9}
 * \parameters{
...
class EPSSMLT : public Integrator {
public:
	EPSSMLT(const Properties &props) : Integrator(props) {
		...
		/* This parameter can be used to specify the samples per pixel used to
		   ...
		   EPSSMLT will handle direct illumination as well */
	...
	EPSSMLT(Stream *stream, InstanceManager *manager)
	...
	bool render(Scene *scene, RenderQueue *queue, const RenderJob *job,
		...
		ref<EPSSMLTSampler> mltSampler = new EPSSMLTSampler(m_config);
...
MTS_IMPLEMENT_CLASS_S(EPSSMLT, false, Integrator)
MTS_EXPORT_PLUGIN(EPSSMLT, "Extended Primary Sample Space MLT");
MTS_NAMESPACE_END
