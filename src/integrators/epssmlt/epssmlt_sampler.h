...
#if !defined(__EPSSMLT_SAMPLER_H)
#define __EPSSMLT_SAMPLER_H
 ...
 */
class EPSSMLTSampler : public Sampler {
public:
	// Construct a new MLT sampler
	EPSSMLTSampler(const EPSSMLTConfiguration &conf);

	/**
	 * \brief Construct a new sampler, which operates on the
	 * same random number generator as \a sampler.
	 */
	EPSSMLTSampler(EPSSMLTSampler *sampler);

	/// Unserialize from a binary data stream
	EPSSMLTSampler(Stream *stream, InstanceManager *manager);
