...
Float PathSampler::generateSeedsSpec(size_t sampleCount, size_t seedCount,
	bool fineGrained, const Bitmap *importanceMap, std::vector<PathSeed> &seeds, const int pathLength) {
	Log(EInfo, "Integrating luminance values over the image plane ("
		SIZE_T_FMT " samples)..", sampleCount);

	BDAssert(!fineGrained);
	BDAssert(m_sensorSampler == m_emitterSampler);
	BDAssert(m_sensorSampler->getClass()->derivesFrom(MTS_CLASS(ReplayableSampler)));
	...
	for (size_t i = 0; i<sampleCount; ++i) {
		...
		const int t = std::min(pathLength + 1, int(m_sensorSampler->next1D() * (pathLength + 2))) + 1;
		const int s = (pathLength + 2) - t;
		
		/* Run the path sampling strategy */
		sampleSplatsSpec(Point2i(-1), splatList, s, t);
		luminance = splatList.luminance;
		splatList.normalize(importanceMap);
	...
	if (tempSeeds.size() == 0){
		// For a certain path length the average illuminance is zero, then push dummy seeds instead
		for (size_t i = 0; i < seedCount; ++i)
			seeds.push_back(PathSeed(-1, (Float)0));
		return (Float)0.f;
	}
	...
	std::vector<PathSeed> tempSeeds_picked;	
	for (size_t i = 0; i<seedCount; ++i)
		tempSeeds_picked.push_back(tempSeeds.at(seedPDF.sample(m_sensorSampler->next1D())));

	/* Sort the seeds to avoid unnecessary rewinds in the ReplayableSampler */
	std::sort(tempSeeds_picked.begin(), tempSeeds_picked.end(), PathSeedSortPredicate());

	for (size_t i = 0; i < seedCount; ++i)
		seeds.push_back(tempSeeds_picked[i]);
...
Float miWeightVM(const Scene *scene, int s, int t, 
	...
	const PathVertex *vsPred3, const PathVertex *vsPred2, const PathVertex *vsPred, const PathVertex *vs,
	const PathVertex *vt, const PathVertex *vtPred, const PathVertex *vtPred2, const PathVertex *vtPred3,
	bool cameraDirConnection, Float gatherRadius, size_t numLightPath, bool useVC, bool useVM, bool useVCMPdf = false){

	Float miWeight = 0.f;
	Float wLight = 0.f, wCamera = 0.f;
	if (cameraDirConnection){
		Float pt = (useVCMPdf) ? vtPred->evalPdf(scene, vtPred2, vt, ERadiance, EArea) : vtPred->evalPdf(scene, vtPred2, vs, ERadiance, EArea);
		Float ps = vsPred->pdf[EImportance];
		...
		Float ptr2 = (useVCMPdf) ? vtPred->evalPdf(scene, vt, vtPred2, EImportance, EArea) : vtPred->evalPdf(scene, vs, vtPred2, EImportance, EArea);
		Float ptPred = vtPred2->pdf[ERadiance];
		...
		Float ps = (useVCMPdf) ? vsPred->evalPdf(scene, vsPred2, vs, EImportance, EArea) : vsPred->evalPdf(scene, vsPred2, vt, EImportance, EArea);
		Float pt = vtPred->pdf[ERadiance];		

		Float psr1 = vt->evalPdf(scene, vtPred, vsPred, ERadiance, EArea);
		Float psr2 = (useVCMPdf) ? vsPred->evalPdf(scene, vs, vsPred2, ERadiance, EArea) : vsPred->evalPdf(scene, vt, vsPred2, ERadiance, EArea);
		Float psPred = vsPred2->pdf[EImportance];
		...
		for (int s = 1; s < (int)m_emitterSubpath.vertexCount(); ++s) {
			...
			if (vs->measure != EDiscrete && wr != NULL && useVC){
				...
				Float miWeight = miWeightVC(m_scene, s, 1, emitterState, sensorState,
					...
					&vt, &vtPred, NULL, NULL,
					gatherRadius, m_lightPathNum, true, useVM);
...
void PathSampler::sampleSplatsUPM(UPMWorkResult *wr,
	...
	Float rejectionProb, size_t clampThreshold, bool useVCMPdf) {
	list.clear();
...
// 					}					
				...
				for (int k = 0; k < searchResults.size(); k++){
					...
					if (cameraDirConnection){
						...
						if (useVCMPdf){
							Float probVCM = vtPred->evalPdf(m_scene, vtPred2, vt, ERadiance) * M_PI * gatherRadius * gatherRadius;
							contrib *= (probVCM > 0.f) ? 1.f / probVCM : 0.f;
							for (int ii = 0; ii < 3; ii++){
								if (!std::isfinite(contrib[ii]) || contrib[ii] < 0)
									contrib[ii] = 0.f;
							}
						}
					}
						...
						if (useVCMPdf){
							Float probVCM = vsPred->evalPdf(m_scene, vsPred2, vs, EImportance) * M_PI * gatherRadius * gatherRadius;
							contrib *= (probVCM > 0.f) ? 1.f / probVCM : 0.f;
							for (int ii = 0; ii < 3; ii++){
								if (!std::isfinite(contrib[ii]) || contrib[ii] < 0)
									contrib[ii] = 0.f;
					...
					// estimate connection probability in unbiased way
					if (!useVCMPdf){
						Float invp = 0.f;
						if (shareShoot){
					...
					Float miWeight = miWeightVM(m_scene, s, t,
						...
						cameraDirConnection, gatherRadius, m_lightPathNum, useVC, useVM, useVCMPdf);
...
// 							continue;
		...
		if (m_sensorSubpath.vertexCount() >= 3){
			int t = 2, s = 0;
			PathVertex
				*vtPred2 = m_sensorSubpath.vertex(0),
				*vtPred = m_sensorSubpath.vertex(1),
				*vt = m_sensorSubpath.vertex(2);
			PathEdge
				*vtEdge = m_sensorSubpath.edgeOrNull(t - 1);
			PathVertex *vs = m_pool.allocVertex();
			PathEdge connectionEdge;
			Spectrum value = Spectrum(0.0f);

			/* If possible, convert 'vt' into an emitter sample */
			if (vt->cast(m_scene, PathVertex::EEmitterSample) && !vt->isDegenerate() && !m_excludeDirectIllum){
				vs->makeEndpoint(m_scene, time, EImportance);
				int depth = s + t - 1;
				int remaining = m_maxDepth - depth;
				value = radianceWeights[t] *
...
void PathSampler::sampleSplatsSpec(const Point2i &offset, SplatList &list, const int SpecifiedNumLightVertices, const int SpecifiedNumEyeVertices) {
	BDAssert(m_technique == EBidirectional);
	const Sensor *sensor = m_scene->getSensor();
	list.clear();
	/* Uniformly sample a scene time */
	...
	for (int s = (int)m_emitterSubpath.vertexCount() - 1; s >= 0; --s) {
		...
		for (int t = maxT; t >= minT; --t) {
			...
			if (s != SpecifiedNumLightVertices || t != SpecifiedNumEyeVertices) continue;
